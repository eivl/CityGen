...
def UI():
    ...
    # create window
    window = cmds.window("cityGen", title="City Generator", width=265,
                         height=430, s=False, menuBar=True)
    ...
    cmds.button(label="Generate", c=partial(cityGen, cityWidth, cityDepth,
                                            maxBuildingHeight, buildingGap,
                                            streetWidth, blockSize,
                                            progressControl))
...
def cityGen(cityWidth, cityDepth, maxBuildingHeight, buildingGap, streetWidth,
            blockSize, progressControl,  *args):
    ...
    maxBuildingHeight = int(cmds.textField(maxBuildingHeight,
                                           q=True, text=True))
    ...
    while(xSpace < cityDepth):
        ...
        while(zSpace < cityWidth):
            ...
            if(len(userBuildings) > 0):
                ...
                # give the building a random scale so it doesn't look so
                # uniform
                buildingScale = randrange_float(.5, 2, .1)
                cmds.scale(buildingScale, buildingScale,
                           buildingScale, building)

                # free the transformations
                cmds.makeIdentity(building, apply=True, t=1, r=1, s=1, n=0)

                # get the bounding box of the building
                buildingX = cmds.xform(building,
                                       q=True,
                                       bb=True)[3] - cmds.xform(building,
                                                                q=True,
                                                                bb=True)[0]
                buildingY = cmds.xform(building,
                                       q=True,
                                       bb=True)[4] - cmds.xform(building,
                                                                q=True,
                                                                bb=True)[1]
                buildingZ = cmds.xform(building,
                                       q=True,
                                       bb=True)[5] - cmds.xform(building,
                                                                q=True,
                                                                bb=True)[2]

                # check if this is the biggest building in the row.If it is,
                # set that as the gap between this and the next column
                if(buildingX > biggestX):
                    biggestX = buildingX

                # check and see if placing the next building will intersect
                # where the street should go
                for z in range(zSpace - 1,
                               zSpace + buildingZ + buildingGap + 1):
                    if(z % blockSize == 0):
                        # create the street geometry
                        streetSegment = cmds.polyPlane(h=streetWidth,
                                                       w=biggestX, sx=1, sy=1)
                        cmds.move(xSpace + (biggestX/2),
                                  0.01, z + (streetWidth/2))
                        cmds.parent(streetSegment[0], "street")
                        zSpace = z + streetWidth + buildingGap
                        break

                cmds.parent(building, "gen_buildings")
                cmds.move((xSpace + (buildingX/2)), 0,
                          (zSpace + (buildingZ/2)))
                ...
                # move it into place
                buildings[counter].moveBuilding((xSpace + (buildingX/2)),
                                                (height / 2),
                                                (zSpace + (buildingZ/2)))

            # update the progress bar
            progressInc = cmds.progressBar(progressControl,
                                           edit=True, maxValue=(cityDepth),
                                           pr=xSpace, vis=True)
        ...
        if xcounter == 1:
            ...
            streetSegment = cmds.polyPlane(h=cityWidth,
                                           w=streetWidth + buildingGap,
                                           sx=1, sy=1)
            cmds.move(xSpace + (streetWidth/2) + biggestX + buildingGap,
                      0.01, cityWidth/2)
            cmds.parent(streetSegment[0], "street")
    ...
    # update the progress bar
    progressInc = cmds.progressBar(progressControl, edit=True, maxValue=100,
                                   pr=0, vis=False)
...
class Building:
    ...
    def create(self):
        ...
        if(self.buildingType == 0):
            cmds.polyCube(w=self.width, d=self.depth, h=self.height,
        ...
        # building type number 2
            ...
            for i in range(0, random.randrange(0, 3)):
                cmds.polyExtrudeFacet(str(self.buildingName) + ".f[1]",
                                      kft=False, ls=(0.8, 0.8, 0))
                cmds.polyExtrudeFacet(str(self.buildingName) + ".f[1]",
                                      kft=False, ltz=30)
        ...
        # building type number 3
            ...
            cmds.polyCube(w=self.width, d=self.depth, h=self.height,
                          sx=self.xDiv, sy=self.yDiv, sz=self.zDiv,
                          n=str(self.buildingName))
            ...
            for i in range(0, 8):
                if(i != 1 and i != 3):
                    sides.append(str(self.buildingName) +
                                 ".f[" + str(self.xDiv * self.yDiv * i) + ":" +
                                 str((self.xDiv * self.yDiv * (i+1)) - 1)+"]")

            # extrude the faces to create windows
            cmds.polyExtrudeFacet(sides[0], sides[1], sides[2], sides[3],
                                  sides[4], sides[5], kft=False,
                                  ls=(0.8, 0.8, 0))
            windows = cmds.ls(sl=True)
            cmds.polyExtrudeFacet(windows[1], windows[2], windows[3],
                                  kft=False, ltz=-0.2)
...
# randrange_float function taken from
# stackoverflow.com/questions/11949179/how-to-get-a-random-float-with-step-in-python
